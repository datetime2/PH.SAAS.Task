(function ($) {
    ...
    function getData(url, level, key, tradetype) {
        var data;
        if (level == -1 || key) {
            $.ajax({
                url: url,
                async: false,
                data: { level: level, key: key,tradetype:tradetype },
                type: "post",
                dataType: "json",
    ...
    function clear(startIndex, index) {
        for (var i = startIndex; i < selectors[index].length; i++) {
            if (selectors[index][i]) {
                ...
                selectors[index][i].append('<option>请选择</option>')
            }
        }
    }
    ...
    function drawSelect(level, key, index) {
        ...
        if (himallLinkageOptions[index].displayWhenNull)
            ...
            selector.empty().show();
        var data = getData(himallLinkageOptions[index].url, level, key, himallLinkageOptions[index].tradetype);
