...
(function ($) {
	...
	function bindEvent(target, opts) {
        ...
        $(target).find('.glyphicon-picture').each(function () {
            ...
            //alert($(this).find('input.hiddenImgSrc').val());
            if (imSrc != '') {
            ...
            $(this).mouseenter(function () {
				...
				var imgstr = '<div class="lg-view-img"><img height="100" src="' + src + '"></div>';
                if (src != '') {
					$('body').append(imgstr);
	...
	$.fn.phupload = function (options, param) {
	    if (typeof options == "string") {
	        return $.fn.phupload.methods[options](this, param);
	    }
	    options = options || {};
	    return this.each(function () {
	        var opts = $.extend({}, $.fn.phupload.defaults, options);
	...
	var renderView = {
	    render: function (opts, container) {
			...
			var html = [
                '<input class="hiddenImgSrc" type="hidden" value="' + opts.displayImgSrc + '"   id="' + opts.imgFieldName + '"  name="' + opts.imgFieldName + '">',
                '<div class="col-sm"><span class="glyphicon glyphicon-picture"></span><input class="file uploadFilebtn ' + opts.isRequired + '" type="file" name="_file"  />',
	...
	$.fn.phupload.defaults = {
	    url: '/PublicOperation/UploadPic',
        displayImgSrc: '',
		title: '图片：',
