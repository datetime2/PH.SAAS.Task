...
(function (factory) {
    ...
    $.jgrid.extend({
        addRowOne: function (rowHtml) {
            var success = false;
            if (rowHtml) {
                this.each(function () {
                    var t = this;
                    $(t.rows[t.rows.length - 1]).after("<tr class=\"" + $(t).jqGrid('getStyleUI', t.p.styleUI + ".base", 'rowBox', true, 'jqgrow ui-row-' + t.p.direction) + "\"><td style=\":height:auto\" colspan=\"" + t.p.colModel.length + "\">" + rowHtml + "</td></tr>");
                    success = true;
                });
            }
            return success;
